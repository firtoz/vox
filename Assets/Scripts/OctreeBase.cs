...
public abstract class OctreeBase<TItem, TNode, TSelf>
    ...
    private Dictionary<int, MeshInfo<TNode>> _meshInfos = new Dictionary<int, MeshInfo<TNode>>();
    ...
    private MeshInfo<TNode> GetMeshInfo(TItem item)
    {
        var meshId = GetItemMeshId(item);

        MeshInfo<TNode> meshInfo;
        ...
        meshInfo = new MeshInfo<TNode>(GetMeshMaterial(meshId));
        _meshInfos.Add(meshId, meshInfo);

        return meshInfo;
    }

    public void NodeAdded(TNode octreeNode, bool updateNeighbours)
    {
    ...
    private void ProcessDrawQueue(MeshInfo<TNode> meshInfo)
    {
        var drawQueue = meshInfo.drawQueue;
    ...
    private static void ProcessRemovalQueue(MeshInfo<TNode> meshInfo)
    {
    ...
    private void AddNodeInternal(TNode octreeNode)
    {
...
if(rems.length>0) {
    ...
    */
    ...
    public void NodeRemoved(TNode octreeNode, bool updateNeighbours)
    {
    ...
    public void UpdateNeighbours(TNode octreeNode)
    {
    ...
    private static void UpdateMeshes(GameObject container, int meshId, MeshInfo<TNode> meshInfo, List<GameObject> objectsForMesh)
    {
    ...
    private static void RenderNewMeshes(GameObject container, int meshId, MeshInfo<TNode> meshInfo,
    ...
    protected abstract int GetItemMeshId(TItem item);
    protected abstract Material GetMeshMaterial(int meshId);

    public bool ItemsBelongInSameMesh(TItem a, TItem b)
    {
